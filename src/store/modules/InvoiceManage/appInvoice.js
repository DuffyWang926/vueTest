...
const appInvoice = {
  ...
  actions: {
    ...
    getAppInvoiceData({ commit,state }, data) {
      ...
      return new Promise((resolve, reject) => {
        getAppInvoice(body).then(response => {
          ...
          checkDetails.cjsj = moment(checkDetails.cjsj*1000).format('YYYY.MM.DD  h:mm:ss')
          checkDetails.fpsj = moment(checkDetails.fpsj*1000).format('YYYY.MM.DD  h:mm:ss')
    ...
    // 控制弹窗开关
    changeCheckDialogue({commit},data){
      commit('CHANGE_CHECK_DIALOGUE',data)
    }
